...
class RegisterRoomRequest(CamelCaseModel):
    room_number: str = Field(
        description="Unique identifier for the room, typically alphanumeric.",
        examples=["101"],
    )
    room_type: str = Field(
        description="Category or class of the room (e.g., STANDARD, SUITE, DELUXE).",
        examples=["STANDARD"],
    )
    capacity: int = Field(
    ...
    price_amount: Decimal = Field(
        description="Base price per night for the room.",
        examples=["100.00"],
    )
    ...
    operational_status: str = Field(
        description=(
            ...
            "(e.g., AVAILABLE, MAINTENANCE, OUT_OF_SERVICE)."
        ),
        examples=["AVAILABLE"],
    )
    availability_start: date | None = Field(
        default=None,
        description=(
            "Start date from when the room is initially available for booking."
        ),
        examples=["2026-04-24"],
    )
    availability_end: date | None = Field(
        default=None,
        description=(
            "End date until when the room is initially available for booking."
        ),
        examples=["2026-04-26"],
    )
...
class UpdateRoomStatusRequest(CamelCaseModel):
    operational_status: str = Field(
        description="The new operational status to assign to the room.",
...
class RoomSummary(CamelCaseModel):
    room_id: str = Field(
        description="Universally Unique Identifier (UUID) of the room.",
        examples=["1fcdc9c0-26d9-4e9f-b80a-3dca3a2fe6c7"],
    )
    room_number: str = Field(
    ...
    room_type: str = Field(
        description="Category or class of the room.",
        examples=["STANDARD"],
    )
    capacity: int = Field(
        description="Maximum number of occupants allowed in the room.",
