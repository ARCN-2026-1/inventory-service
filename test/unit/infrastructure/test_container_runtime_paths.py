...
def test_base_compose_pins_working_dir_and_pythonpath_for_all_services() -> None:
    compose = (PROJECT_ROOT / "docker-compose.yml").read_text(encoding="utf-8")
    assert "x-inventory-runtime: &inventory-runtime" in compose
    ...
    for service_name in ("inventory-migration", "inventory-api", "inventory-worker"):
        ...
        assert "<<: *inventory-runtime" in service_block
